	...
	<div class="container">
		...
		<div class="zhanwei"></div>
			<form class="mui-input-group">
				...
				<div class="mui-input-row">
					<label>截止时间</label>
					<input v-model="stoptime" type="text" class="mui-input-clear" placeholder="2021-3-30 19:00">
				</div>
			</form>
			...
			<div style="text-align: center;">
				 <el-button type="primary" @click="submit(),isShow=true">添加</el-button>
			</div>
...
<script>
	...
	import { addGroupBying } from '../../network/groupBying.js'
	export default {
		name:'AddGroupBying',
		data() {
			return {
		...
		methods:{
			...
			handleAvatarSuccess(response,file){
				...
				addGroupBying(this.$route.query.account,this.title,this.image,this.detail,this.stoptime,this.contact).then(res => {
					this.$router.back()
				})
			},
			submit(){
				if(!(this.title&& this.stoptime&& this.contact)){
					this.msg = "标题，联系方式或截至时间不能为空"
					this.isShow = true;
					return;
				}
